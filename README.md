...
A interface principal do usuário é a classe AgendaUI.
...
### 1. Lista Duplamente Encadeada Circular (Classe Agenda)
As tarefas diárias de uma agenda são organizadas em uma Lista Duplamente Encadeada Circular composta por nós da classe Tarefa:
...
### 2. Lista Encadeada Simples (Classe Histórico)
O histórico de produtividade armazena os relatórios diários de dias anteriores por meio de uma Lista Encadeada Simples de nós do tipo Relatorio:
...
### 3. Persistência de Dados (Classe GerenciadorBanco)
...
* **Botão Finalizar Ciclo do Dia**:
  1. Avalia as tarefas da agenda atual e gera uma instância de Relatorio estatístico do dia.
  2. Adiciona o relatório à lista encadeada do Historico.
  3. Cria e registra uma nova Agenda correspondente ao dia seguinte (LocalDate.now() + 1 dia).
...
O sistema possui uma regra de expiração inteligente na classe Gerenciador:
...
**Nota sobre as dependências:** Para respeitar o limite de tamanho de envio de arquivos do SIGAA (10MB), o arquivo `.jar` do driver do banco de dados SQLite foi omitido do anexo. O passo 1 abaixo fará o download automático dessa dependência.

Certifique-se de estar na raiz do diretório do projeto e execute os seguintes comandos no terminal:

### 1. Download da Dependência (SQLite)
Crie a pasta para as bibliotecas e faça o download do driver JDBC do SQLite diretamente do repositório Maven:

**Linux / macOS:**
```bash
mkdir -p libs

curl -L -o libs/sqlite-jdbc-3.53.2.0.jar https://repo1.maven.org/maven2/org/xerial/sqlite-jdbc/3.53.2.0/sqlite-jdbc-3.53.2.0.jar
```
...
### 3. Execução da Interface Principal (AgendaUI)
...
O projeto possui um conjunto de testes unitários na classe TestGerenciador que valida os comportamentos de alteração e expiração de tarefas.
